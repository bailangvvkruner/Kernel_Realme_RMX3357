...
u64 _slide_get_boost_load(int cpu) {
    ...
    if ((sched_assist_scene(SA_SLIDE) || sched_assist_scene(SA_INPUT) || sched_assist_scene(SA_LAUNCHER_SI) || sched_assist_scene(SA_ANIM)) && ux_task_load[cpu]) {
...
{
	if (sysctl_sched_assist_enabled && (sched_assist_scene(SA_SLIDE) || sched_assist_scene(SA_LAUNCHER_SI) || sched_assist_scene(SA_INPUT) || sched_assist_scene(SA_ANIM)) && ux_task_load[cpu]) {
		*boosted = true;
